...
const CommunicationChannels = () => {
    ...
    const [info , setInfo] = useState({
        work_channels:{
            ...journeyInputFields['work_channels']
        }
    })


    const addOrRemoveCommunicationChannels = (id) => {
        const prevCommunicationChannels = [...info.work_channels.work_channels.value]
        if (prevCommunicationChannels.find(id2 => id2 == id)) {
            ...
            setInfo(prev => ({
                ...
                work_channels:{
                    ...prev.work_channels,
                    work_channels:{
                        ...prev.work_channels.work_channels,
    ...
    const sendInfo = () => {
        console.log(info);
        if(info.work_channels.work_channels.value.length!=0){
            journeyDispatch({function:'setSection' , section:info})
            console.log(info);
            console.log(journeyInputFields['work_channels']);
        ...
        <Animated.View entering={SlideInDown} exiting={SlideOutDown} className="relative top-0 left-0 w-screen h-full justify-start items-center bg-white rounded-tl-[50px] rounded-tr-[50px] z-10">
            <View className='p-6'>
                ...
                <View className='w-[300px] h-[213px] flex justify-center items-center '>
                    <Image className="absolute max-w-[212px] max-h-[153px]" source={require('./../../../assets/Images/TeamWork.png')} />
                    <Image className="absolute -top-10 -right-10 max-w-[186px] max-h-[142px]" source={require('./../../../assets/Images/TheTeam.png')} />
                    <View className="absolute top-3 left-5 p-3 rounded-full justify-center items-center bg-black">
                        <TeamNetwork />
                    </View>
                    <View className="absolute -bottom-0 right-5 p-2 rounded-[10px] justify-center items-center bg-black">
                        <Messages />
                    </View>
                </View>
            </View>
            ...
            {
                    info.work_channels.work_channels.error &&
                    <View>
                        <Text className="text-left text-red-700">{info.work_channels.work_channels.error}</Text>
                    </View>
                }
                <FlatList
                    style={{ width: '100%' }}
                    ...
                    renderItem={item =>
                        <TouchableOpacity
                            ...
                            style={{
                                borderColor: info.work_channels.work_channels.value.find(id => id == item.item.id) ? '#1776F2' : '#bfbfbf',
                                backgroundColor: info.work_channels.work_channels.value.find(id => id == item.item.id) ? '#1776F2' : '#ffffff',

                            }}
                        >
                            <Text className="text-black"
                                style={{
                                    color: info.work_channels.work_channels.value.find(id => id == item.item.id) ? '#ffffff' : '#000000'
                                }}
                            >{item.item.name}</Text>
                        </TouchableOpacity>
                    }
